    ...
    func get(from url: URL, compleation: @escaping (HTTPClientResult) -> Void) {
        session.dataTask(with: url) { data, response, error in
            if let error {
                ...
                compleation(.succes(data, response))
            } else {
                compleation(.failure(UnexpectedValueRepresentation()))
            }
        }.resume()
    }
}
...
class URLSessionHTTPClientTests: XCTestCase {
    ...
    override func tearDown() {
        ...
        URLProtocolStub.stoptInterceptingRequest()
    }
    
    func test_getFromURLPperformsGETRequstsWithURL() {
        let url = anyURL()
    ...
    func test_get_fromURL_failsOnRequestError() {
        ...
        let receviedError = resultErrorFor(data: nil, response: nil, error: requestError)
 
        XCTAssertEqual((receviedError as NSError?)?.code, requestError.code)
    }
    ...
    private func resultValuesFor(data: Data?, response: URLResponse?, error: Error?, file: StaticString = #filePath, line: UInt = #line) -> (data: Data, response: HTTPURLResponse)? {
        ...
        switch result {
        case let .succes(data, respose):
            return (data, respose)
        default:
            XCTFail("Expected succes, got \(result) insted", file: file, line: line)
            return nil
        }
    }
    ...
    private class URLProtocolStub: URLProtocol {
        ...
        static func stoptInterceptingRequest() {
